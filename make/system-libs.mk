...
$(D)/ffmpeg-$(FFMPEG_VER): alsa-lib fribidi libao libxml2 openssl $(ARCHIVE)/ffmpeg-$(FFMPEG_VER).tar.bz2 $(PATCHES)/ffmpeg-hds.patch $(PATCHES)/.rebuild.ffmpeg $(D)/libbluray | $(TARGETPREFIX)
	$(REMOVE)/ffmpeg-$(FFMPEG_VER) $(PKGPREFIX)
	$(UNTAR)/ffmpeg-$(FFMPEG_VER).tar.bz2
	set -e; cd $(BUILD_TMP)/ffmpeg-$(FFMPEG_VER); \
		: $(PATCH)/fmpeg-2.6.patch; \
		$(PATCH)/ffmpeg-hds-2.6.patch; \
		$(PATCH)/ffmpeg-aac-2.6.patch; \
		./configure \
			--disable-ffserver \
			--disable-ffplay \
			--disable-ffprobe \
